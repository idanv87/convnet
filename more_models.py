...
class ConvNet(nn.Module):
    def __init__(self, activation=nn.ReLU()):
        ...
        self.layer1 = nn.Conv2d(in_channels=1, out_channels=40, kernel_size=3, stride=1)
        self.layer2 = nn.Conv2d(in_channels=40, out_channels=60, kernel_size=3, stride=1)
...
class conv_deeponet_f(nn.Module):
    def __init__(self, dim,f_shape, domain_shape,  p):
        ...
        n_layers = 4
        self.n = p
        self.attention1=SelfAttention2(input_dims=[2,2,2], hidden_dim=1)
        self.attention2=SelfAttention2(input_dims=[1,1,1], hidden_dim=1)
        ...
        self.linear2=FullyConnectedLayer(225,80,activation=nn.Tanh())
    ...
    def forward(self, X):
        y,f,dom, mask=X
        x=torch.stack((f,dom.view(-1,225)),dim=-1)
        branch=self.linear2(self.attention1(x,x,x,mask).squeeze())
        # f=f.view(-1,1, 15,15)

        # branch1= self.branch1(f)
        trunk1=self.trunk(y)
        trunk=self.linear3(self.attention2(dom.view(-1,225).unsqueeze(-1),dom.view(-1,225).unsqueeze(-1),trunk1.unsqueeze(-1),mask).squeeze())
